...
CloudFront 建立完成後（§3）會給一段建議的 bucket policy，**但 AWS Console
產生的範本只給 `s3:GetObject`，必須再加上 `s3:ListBucket` 才能讓「不存在的
key」正確回 404**。完整 policy：
...
- `s3:GetObject` 是 **object 層級**權限，Resource 用 `<bucket-name>/*`。
- `s3:ListBucket` 是 **bucket 層級**權限，Resource 用 `<bucket-name>`
  （**不含** `/*`）。兩者必須同時列在 Resource 陣列裡。
- **為何需要 `s3:ListBucket`**：S3 預設行為是當呼叫者**沒有**
  `s3:ListBucket` 權限時，對「不存在的 key」回 `403 AccessDenied`
  （刻意隱藏 key 的存在與否）；只有授權 `s3:ListBucket` 之後，S3 才會對
  缺檔回真正的 `404 NoSuchKey`。CloudFront 會把 S3 的錯誤原樣透傳，
  所以這個權限直接決定瀏覽器收到 403 還是 404。
- 若只給 `s3:GetObject`，會出現兩個症狀：
  1. 對缺檔請求（例如 `/data/videos/UNKNOWN.meta.json`）瀏覽器收到一段
     `<Error><Code>AccessDenied</Code>…</Error>` 的 XML。
  2. [src/api/client.ts](../src/api/client.ts) 的 `NotFoundError` 流程
     判斷 `res.status === 404`，但 OAC 回的是 403，所以 "Archive not
     yet available" UI 永遠不會觸發。
